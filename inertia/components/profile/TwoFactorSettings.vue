...
const confirmSetup = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      verificationError.value = errorData.message || t('profile.invalidVerificationCode')
    }
  } catch (error) {
    verificationError.value = t('profile.errorVerifyingCode')
  } finally {
    loading.value = false
  }
}
...
const disableTwoFa = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const errorData = await response.json()
      disableError.value = errorData.message || t('profile.invalidVerificationCode')
    }
  } catch (error) {
    disableError.value = t('profile.errorDisabling2FA')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="bg-white rounded-lg shadow-md p-6">
    <h3 class="text-lg mb-4">{{ t('profile.twoFactorAuth') }}</h3>
    
    <!-- 2FA Status -->
    <div class="mb-4">
      <div class="flex items-center justify-between">
        <div>
          <p class="font-medium">{{ is2faEnabled ? t('profile.twoFactorEnabled') : t('profile.twoFactorDisabled') }}</p>
          <p class="text-sm text-gray-600">
            {{ is2faEnabled ? t('profile.twoFactorProtected') : t('profile.twoFactorDescription') }}
          </p>
        </div>
        <div class="flex items-center">
          <span class="w-3 h-3 rounded-full mr-2" :class="is2faEnabled ? 'bg-green-500' : 'bg-red-500'"></span>
          <span class="text-sm font-medium">{{ is2faEnabled ? t('profile.twoFactorActive') : t('profile.twoFactorInactive') }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="!is2faEnabled && !setupMode" class="space-y-4">
      <p class="text-sm text-gray-600">
        {{ t('profile.twoFactorSecurityDescription') }}
      </p>
      <button
        @click="enableTwoFa"
        :disabled="loading"
      ...
      >
        {{ loading ? t('profile.settingUp') : t('profile.enableTwoFactor') }}
      </button>
    </div>
    ...
    <div v-if="setupMode" class="space-y-4">
      <div class="bg-blue-50 p-4 rounded-lg">
        <h4 class="font-medium mb-2">{{ t('profile.stepInstallApp') }}</h4>
        <p class="text-sm text-gray-600 mb-2">
          {{ t('profile.installAppDescription') }}
        </p>
      </div>

      <div class="bg-blue-50 p-4 rounded-lg">
        <h4 class="font-medium mb-2">{{ t('profile.stepScanQR') }}</h4>
        <p class="text-sm text-gray-600 mb-4">
          {{ t('profile.scanQRDescription') }}
        </p>
      ...
      <div class="bg-yellow-50 p-4 rounded-lg">
        <h4 class="font-medium mb-2">{{ t('profile.recoveryCodes') }}</h4>
        <p class="text-sm text-gray-600 mb-3">
          {{ t('profile.recoveryCodesDescription') }}
        </p>
      ...
      <div class="bg-green-50 p-4 rounded-lg">
        <h4 class="font-medium mb-2">{{ t('profile.stepVerifySetup') }}</h4>
        <p class="text-sm text-gray-600 mb-3">
          {{ t('profile.verifySetupDescription') }}
        </p>
        ...
        <form @submit.prevent="confirmSetup" class="space-y-3">
          ...
          <div class="flex gap-2">
            ...
            >
              {{ loading ? t('profile.verifying') : t('profile.verifyAndEnable') }}
            </button>
          </div>
        </form>
      </div>
    ...
    <div v-if="is2faEnabled && !setupMode" class="space-y-4">
      <div class="bg-green-50 p-4 rounded-lg">
        <p class="text-sm text-gray-600 mb-2">
          {{ t('profile.twoFactorActiveMessage') }}
        </p>
        <p class="text-sm text-gray-600">
          {{ t('profile.recoveryCodesRemaining') }} <span class="font-medium">{{ recoveryCodesCount }}</span>
        </p>
      </div>
      ...
      <div v-if="showDisableForm" class="bg-red-50 p-4 rounded-lg">
        <h4 class="font-medium mb-2 text-red-800">{{ t('profile.disableTwoFactorConfirm') }}</h4>
        <p class="text-sm text-red-700 mb-3">
          {{ t('profile.disableTwoFactorDescription') }}
        </p>
        ...
        <form @submit.prevent="disableTwoFa" class="space-y-3">
          ...
          <div class="flex gap-2">
            ...
            >
              {{ loading ? t('profile.disabling') : t('profile.disableTwoFactor') }}
            </button>
          </div>
        </form>
      </div>
